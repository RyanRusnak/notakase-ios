...
/// JSON-RPC 2.0 server over stdio, implementing the subset of the Model
/// Context Protocol that exposes tools. Newline-delimited JSON — one message
/// per line on stdin, one per line on stdout. Mirrors todarchy-mcp.
final class MCPServer {
    ...
    private func handle(_ line: String) {
        ...
        do {
            ...
            if !isNotification { writeResult(id: id, result: result) }
        } catch let error as MCPError {
            if !isNotification { writeError(id: id, code: error.code, message: error.message) }
        } catch {
            if !isNotification {
    ...
    private func dispatch(method: String, params: [String: Any]) throws -> Any {
        ...
        case "initialize":
            return [
                ...
                "serverInfo": ["name": "notakase-mcp", "version": "0.1.0"],
            ]
